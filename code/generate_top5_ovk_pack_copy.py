import numpy as np
import pandas as pd
import zipfile, shutil, base64, html, warnings, time
...
# -----------------------------
SRC_ZIP = Path('/mnt/data/data.zip')
...
for t in range(len(xs)):
    ...
    Lcov.append(Lc)
Lcov = np.array(Lcov)
tau_draws = np.empty((Bstate, len(xs)))
diag_draws = np.empty((Bstate, R, len(xs)))
for b in range(Bstate):
    eps = rng.normal(size=(len(xs), dstate))
    Xdraw = xs + np.einsum('tij,tj->ti', Lcov, eps)
    Ad = matrix_series_from_state(Xdraw, R)
    tau_draws[b] = np.trace(Ad, axis1=1, axis2=2) / R
    for r in range(R):
        diag_draws[b, r] = Ad[:, r, r]
